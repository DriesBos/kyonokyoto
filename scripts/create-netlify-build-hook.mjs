...
const hookName =
  process.argv
    ...
    ?.slice("--name=".length) ?? "Scheduled crawl deploy";
...
async function netlifyRequest(token, path, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Netlify request failed (${response.status}) for ${path}: ${errorText}`,
    );
  }
...
const netlifyToken =
  process.env.NETLIFY_AUTH_TOKEN || sharedEnv.NETLIFY_AUTH_TOKEN;
const netlifySiteId = process.env.NETLIFY_SITE_ID || sharedEnv.NETLIFY_SITE_ID;

if (!netlifyToken || !netlifySiteId) {
  throw new Error(
    "Missing NETLIFY_AUTH_TOKEN or NETLIFY_SITE_ID in .env or shell environment",
  );
}

const existingHooks = await netlifyRequest(
  netlifyToken,
  `/sites/${netlifySiteId}/build_hooks`,
);
let buildHook = existingHooks.find((hook) => hook.title === hookName) ?? null;

if (!buildHook) {
  buildHook = await netlifyRequest(
    netlifyToken,
    `/sites/${netlifySiteId}/build_hooks`,
    {
      method: "POST",
...
const updatedCrawlerEnv = upsertEnvValue(
  crawlerEnvContents,
