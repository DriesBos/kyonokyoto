...
2026-08-19: Approved production containment archived 17 exact false-positive events without deleting rows: 3 SAMAC listing/permanent rows, 2 NMOA source-name rows, 1 hitoto placeholder, 5 MOT template-title rows, 4 Whitestone listing/news rows, and 2 HKAC programme-index rows. All 17 statuses re-queried as `archived`. Cache-bypassed Astro runtime checks for Kyoto, Osaka, Tokyo, and Hong Kong returned none of the contained titles. Rows remain recoverable; source tuning and render-contract work are still pending.

2026-08-19: Authorized targeted production recrawls after deploy `da71bc9`. Clean: NAKKA `d6f7b5ba-43f1-4968-97f1-dcbb54efdb4e` saved 1; White Cube `21a85e99-89c1-49fb-b702-ad8f5ed14165` saved 3; SAMAC `a468f4c7-3b07-43dd-a045-b8b5e81b01ea` saved 5 but exposed listing/permanent pages as plausible events; NMOA `609e33db-09ca-4f67-8d77-8944710a0c18` saved 2 but its broad `h1` selector still chose the site heading. Review-only: hitoto `4a6df52b-8aa1-4efb-b3e9-24ec5f535684` saved 1 and skipped 3 missing-image rows; Whitestone `faeaca24-cebb-4ec2-8316-ffaa470a1ba1` found no current detail; MOT `2f13507d-9fe0-4eb5-a9f8-3d21da552914` and HKAC `62623909-b1ce-4869-b143-0c5418139d54` could not render locally because configured Python 3.9 is below Crawl4AI's Python 3.10 minimum. Live Astro still showed plausible-but-wrong SAMAC, NMOA, Whitestone, and HKAC cards. Further source tuning and a production render-contract/quarantine plan are required.

2026-08-19: Local health repair only; no crawl, database write, deploy, or source approval. Astro restored as Netlify production target. Date preflight now rejects impossible canonical schedules, crawl metrics separate created rows from updates/archives, and focused source config prevents known title/listing leaks for MOT, NAKKA, NMOA, hitoto, SAMAC, White Cube, Whitestone, and HKAC. Targeted production recrawls remain required before card approval; existing malformed rows remain unchanged until those crawls run.

2026-07-23: Verified autonomous VPS crawl after deploy: scheduled Kyoto cycle covered all 51 sources, finished with 23 success, 28 review-only partial success, and 0 failures; every run recorded `trigger_type: scheduled`. Economical landing backfill skipped 10 sources whose published images already had dimensions, then retried only 7 incomplete sources through static HTML (`CRAWL4AI_RENDER_MODE=never`), using no renderer or LLM extraction. Final landing payload QA contains all 5 requested Kyoto, 8 Tokyo, and 4 Osaka sources. Taka Ishii run `29515808-a1af-4677-9805-b9faf35ed783` cleanly reported no current events after skipping 12 past Kyoto exhibitions. Sokyo run `793d3792-f9cb-4149-8f4c-ff0d59175c95` saved its current bilingual event with measured media; one retry-created duplicate was archived and remains recoverable. Hyogo run `f1efd20f-0945-4039-a0b7-20dfbcef4c37` saved 6 Kobe events with measured media and skipped 3 missing-image events. Translation audit passes for all 434 published events.
...
2026-07-17: Explicit approval promoted `m-plus` to public. Discovery now keeps only official On Now and Upcoming cards, excluding Online and Past; detail extraction uses the English title and authoritative M+ date block, including open-ended `Ongoing` exhibitions. Live static listing/detail probes and focused crawler coverage pass; production sync/recrawl remains pending.

2026-07-17: Hong Kong ready for renewed QA after deploy `82cb959`. Full 36-source VPS cycle completed with 0 failed sources and a 397/397 translation audit; five sources remain review-level (`villepin`, `hanart-tz-gallery`, `galerie-du-monde`, `ora-ora`, and `grotto-fine-art`). Targeted Oi! crawl `597c3bc8-64e7-43aa-bf6c-1e2422f11458` saved 3 dated/open-ended programmes, skipped 3 undated permanent projects, stored exactly 1 image per published event, and finished with 400/400 translations. Production migration `20260717015643_allow_open_ended_event_schedules` aligned the event schedule constraint with shared code. Stale `Lightbox Prompt` and `Web Content Display` rows were archived explicitly because review-level crawls do not auto-archive. Final database audit: Hong Kong Palace Museum has 9 published events with 0 duplicate/OG/logo/Iogo/map media; David Zwirner has 2 with 0 duplicate/known black-divider media; Oi! has 3 with exactly 1 image each. Ready for renewed card/media QA.
...
2026-07-17: Full 36-source Hong Kong VPS cycle on `0466c92` finished with 0 failed sources and a clean 397-event translation audit, but returned degraded because five sources remain review-level (`villepin`, `hanart-tz-gallery`, `galerie-du-monde`, `ora-ora`, and `grotto-fine-art`). Hong Kong Palace Museum saved 9 events with 0 OG/logo/map media; David Zwirner saved 2 events with 0 known black-divider media. Oi! discovered six official programme details but saved none because Crawl4AI omitted its banner heading and day-first dotted dates were unsupported. Follow-up uses the document title, parses `From D.M.YYYY`, `D.M.YYYY onward`, and dotted ranges, preserves open-ended schedules, and still stores only the first desktop image. Targeted Oi! VPS recrawl pending.
...
2026-07-16: Galerie du Monde now always removes the first extracted image. The previous alternate-only guard retained an unusable poster when it was the event's sole image; focused coverage now checks both multi-image and poster-only pages. Targeted production recrawl pending.

2026-07-16: Hong Kong Palace Museum extraction now drops the publisher's `HKJC_YOTH_Iogo-*` sponsor image; its filename misspells `logo` with a capital `I`, bypassing the shared logo filter. Focused coverage includes both `logo` and `Iogo` sponsor filenames plus the existing map-thumbnail exclusion. Targeted production recrawl pending.

2026-07-16: Hong Kong Palace Museum title extraction now removes the source's fixed `Hong Kong Palace Museum | ` heading prefix before persistence, leaving the exhibition title unchanged otherwise. Deploy `0692a97` passed all checks; a durable targeted VPS recrawl waited behind the scheduled Kyoto cycle, then returned cleanly and saved all 9 events. Database verification found 0 prefixed canonical titles and 0 prefixed EN/JA translations; local Hong Kong HTML also contains 0 prefixed event titles. Post-crawl translation audit passed 396 published events with 0 missing EN/JA translations. Ready for renewed title QA.
...
2026-07-16: Landing slider hardening now measures every configured landing-source image during crawls and persists dimensions for viewport-specific quality gating. Fullscreen slides require at least 1.5 source pixels per rendered CSS pixel, target up to 2x density, and use exact-size Netlify Image CDN cover transforms. Images can qualify on mobile while being rejected on desktop; undersized/unknown candidates fall back to the solid landing. Production migration `20260716074226_add_event_image_metadata` applied and public queries verified. Targeted VPS backfill completed for all nine landing sources: KCUA 1 event/3 probes (`source_ok`), Artro 0/0 (`source_empty`), NMAO 2/10 (`source_ok`), Abeno Harukas 4/8 (`source_needs_review`), Mori 6/12 (`source_ok`), Tokyo Photographic Art Museum 4/16 (`source_needs_review`), David Zwirner 2/7 (`source_ok`), Kiang Malingue 1/5 (`source_ok`), and Gallery EXIT 1/5 (`source_ok`); every probe succeeded. Stored-dimension audit found desktop-eligible images at KCUA (1), TOP Museum (3), David Zwirner (7), and Kiang Malingue (5); NMAO adds 2 mobile-only images. Artro has no current event, while current Abeno, Mori, and Gallery EXIT images are correctly rejected as undersized. TOP Museum still discovers news rows and needs source cleanup; final visual landing QA remains pending. Post-backfill translation audit passed 401 published events with 0 missing EN/JA translations.
