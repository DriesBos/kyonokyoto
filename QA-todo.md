...
2026-07-16: Added beta `kiang-malingue-hong-kong` from the official present/future exhibition listings. Discovery keeps only cards whose venue is `10 Sik On Street, Wanchai, Hong Kong`, excluding New York rows; detail extraction handles the publisher's two-digit dotted date ranges. Live official listing currently shows one Hong Kong exhibition, `Wang Xiaoqu | Dwelling in Mirrors`, through 2026-08-22. `tai-kwun` remains excluded because its current `robots.txt` still declares `User-agent: *` and `Disallow: /`, including `/en/programme`. Kiang source sync, VPS crawl, and card/media QA pending.
