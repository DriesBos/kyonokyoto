...
export default async function CityEventsPage({ city, locale }: { city: CityConfig; locale: AppLocale }) {
  ...
  return (
    <>
      {landingCandidates.length > 0 && <BlocksLanding candidates={landingCandidates} cityLabel={city.label} />}
      <MapExperience
        header={<ExperienceHeader city={city.slug} locale={locale} brandLabel={city.brandLabel} categories={categories} />}
        cityLabel={city.label}
        locale={locale}
        mapCenter={city.mapCenter}
        sources={mapSources}
        apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ?? process.env.PUBLIC_GOOGLE_MAPS_API_KEY}
      ...
      >
        <main className={styles.main} data-main-content>
          ...
          <SiteFooter compact className={styles.blocksFooter} cityLabel={city.label} locale={locale} />
        </main>
      </MapExperience>
