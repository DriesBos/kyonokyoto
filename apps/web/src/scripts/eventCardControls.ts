...
const hasActiveCardBefore = (card) =>
  Array.from(document.querySelectorAll(`${cardSelector}[data-active='true']`)).some(
    (activeCard) => {
...
const getMediaHeight = (card, isActive) => {
  ...
  return styles
    .getPropertyValue(
      isActive ? '--event-card-media-row-height-active' : '--event-card-media-row-height',
    )
    .trim();
};
...
export const initEventCardControls = () => {
  ...
  document.addEventListener('event-card:activate', (event) => {
    ...
    const targetCard = cards.find((card) => {
      if (!(card instanceof HTMLElement)) return false;
      if (card.hidden || card.closest('[hidden]') || card.getClientRects().length === 0)
        return false;
